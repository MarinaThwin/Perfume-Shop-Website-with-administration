...
$page=isset($_GET['page']) ? $_GET['page']:1; //pagination button
?>
<!DOCTYPE html>
<html lang="en">
<head>
...
<div class="col-md-12 bd ">
    <div class="row">
        <?Php 
            $getquery="SELECT * from category where catid='$cid'";
            $go_query=mysqli_query($connection,$getquery);
                        while($row=mysqli_fetch_array($go_query)){
                        
                            $catname=$row['catname'];
        ?>
        <h2 class="text-uppercase text-dark text-center mt-3 mb-3">~~~~<?php echo $catname; ?>~~~~</h2>
        <?Php } ?>
        <?php
            $start=($page-1)*$perpage;
            $query="SELECT * from product where categoryid='$cid' limit $start,$perpage";
            $go_query=mysqli_query($connection,$query);
                while($row=mysqli_fetch_array($go_query)){
        ...
        ?>
        <div class="container col-md-3 mb-2 d-flex justify-content-around">
...
<?php
            for($i=1;$i<=$num;$i++){
                if($i==$page)
                {
                echo "<li class='page-item-native'><a class='page-link' href='product.php?page={$i}'>{$i}</a></li>";
                }
                else
                {
                    echo "<li class='page-item'><a class='page-link' href='product.php?cid=$cid&&page={$i}'>{$i}</a></li>";
                }
            }
            ?>
</ul>
...
<div class="container col-md-2 mb-2 bd1">
        <div>
            <a href="product.php?cid=<?php echo $catid; ?>" class="btn mt-3"><img src="Photo/<?php echo $catphoto; ?>" class="img-fluid d-block w-100" style="width:150px; height:250px;"></a>
            <h4 class="text-center"><?php echo $catname; ?></h4>
        </div>
      </div>
